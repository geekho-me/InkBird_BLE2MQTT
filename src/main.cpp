...
class MyClientCallback : public BLEClientCallbacks {
  void onConnect(BLEClient* pclient) {
    ESP_LOGI("BLE", "onConnect");
  }

  void onDisconnect(BLEClient* pclient) {
    ESP_LOGI("BLE", "onDisconnect");
    connected = false;
  }
};
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
  ...
  void onResult(BLEAdvertisedDevice advertisedDevice) {
    ESP_LOGD("BLE", "-------------");
    ESP_LOGD("BLE", "Found Device:")
    ESP_LOGD("BLE", "Address:   %s", advertisedDevice.getAddress().toString().c_str());
    ESP_LOGD("BLE", "Name:      %s", advertisedDevice.getName().c_str());
    ESP_LOGD("BLE", "RSSI:      %s", String((int)advertisedDevice.getRSSI()).c_str());
    ESP_LOGD("BLE", "-------------");
    
    // We have found a device, let us now see if it contains the service we are looking for.
    if (advertisedDevice.haveServiceUUID() && advertisedDevice.getServiceUUID().equals(serviceUUID)) {
      ESP_LOGI("BLE", "Found our device!  address: ");
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    ESP_LOGI("BBQ", "Connecting to Wifi...");
  }
...
void loop() {
  ...
  if (connected != true) {
    ESP_LOGD("BLE", "Scanning for BLE devices...");
    
    if (mqttIsConnected()) {
      mqttclient.publish("inkbird/BLE", "Disconnected");
    }
    ...
    pBLEScan->start(30);
      ...
      if (mqttIsConnected()) {
        mqttclient.publish("inkbird/BLE", "Connected");
      }
  }
